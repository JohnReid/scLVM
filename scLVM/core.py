...
class scLVM(object):
  ...
  def set_tech_noise(self, tech_noise):
    ...
    """
    print('Setting tech_noise')
    assert tech_noise.shape[0] == self.G, 'scLVM:: tech_noise dimension mismatch'
  ...
  def varianceDecomposition(self, K=None, tech_noise=None, idx=None, i0=None, i1=None, max_iter=10, verbose=False):
    ...
    count  = 0
    # delta optimization might be more efficient
    ...
    if geneID is not None:
      var_info['geneID'] = SP.array(geneID)
    Ystar_info = {'gene_idx': idx, 'conv': conv}
    if geneID is not None:
      Ystar_info['geneID'] = SP.array(geneID)
  ...
  def getCorrectedExpression(self, rand_eff_ids=None):
    ...
    Ystd = self.Y - self.Y.mean(0)
    Ystd /= self.Y.std(0)
    # copy.copy(self.Y[:,self.Ystar_info['gene_idx']])
    Ycorr = Ystd[:, self.Ystar_info['gene_idx']]
    for i in rand_eff_ids:
      Ycorr -= self.Ystar[i]
    # bring back to original scale
    Ycorr *= self.Y[:, self.Ystar_info['gene_idx']].std(0)
...
#		print QTL
    ...
    lmm_params = {'covs': SP.ones(
        [self.N, 1]), 'NumIntervalsDeltaAlt': 100, 'NumIntervalsDelta0': 100, 'searchDelta': True}
    ...
    for ids in idx:
      ...
      if K is not None:
        ...
        _K = None
      lm = QTL.test_lmm(Ystd, Ystd[:, ids:ids + 1],
                        K=_K, verbose=False, **lmm_params)
      pv[count, :]   = lm.getPv()[0, :]
